...
const initialFormData = {
  ...
  recipient: "Standard",
  volume: 330,
  unite: { nom: "mililitres", symbole: "ml" },
  prix: 500,
  imgURL: "",
  ingredients: [],
  calories: 0,
...
const useBoissonFormStore = create((set, get) => ({
  // État du formulaire
  ...
  validateForm: () => {
    ...
    // Validation dénomination (seul champ obligatoire)
    if (!formData.denomination.trim()) {
    ...
    if (formData.volume <= 0) {
      errors.volume = "Le volume doit être supérieur à 0";
    }
    if (formData.imgURL && !isValidUrl(formData.imgURL)) {
  ...
  fillDefaults: () =>
    set((state) => ({
      formData: {
        ...state.formData,
        recipient: state.formData.recipient || "Standard",
        volume: state.formData.volume || 330,
        unite: state.formData.unite || { nom: "mililitres", symbole: "ml" },
        prix: state.formData.prix || 500,
...
export default useBoissonFormStore;
