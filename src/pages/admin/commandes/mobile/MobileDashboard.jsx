/**
 * MobileDashboard.jsx
 * Dashboard mobile pour les commandes avec 2 tabs:
 * - Stats du jour (Ventes, Emplacements, Encaissements, Tendances)
 * - Commandes du jour
 */
...
/**
 * Tab Stats du jour avec 4 cards verticales
 */
const StatsTab = () => {
  const { statistiques, loading, isArchiving } = useCommandeStatistiques();
  const { statistiquesWeek } = useCommandeStatistiquesWeek();
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-3 space-y-3">
        ...
        {/* Card 3: Encaissements (avec comparaison) */}
        <EncaissementsCard
          statistiques={statistiques}
          statistiquesWeek={statistiquesWeek}
        />

        {/* Card 4: Tendances (Carrousel de charts) */}
        <TendancesCard
          statistiques={statistiques}
          statistiquesWeek={statistiquesWeek}
        />
              ...
              <AnimatePresence mode="popLayout">
                {articles.map((article) => (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: 10 }}
                    className="grid grid-cols-12 gap-2 text-xs py-1.5 border-b last:border-0">
...
const EncaissementsCard = ({ statistiques, statistiquesWeek }) => {
  ...
  // Récupérer les encaissements d'hier
...
/**
 * Card 4: Tendances (Carrousel de 4 charts)
 */
...
const PercentageBadge = ({ percentage }) => {
  ...
  return (
    <span className={`${color} font-semibold flex items-center gap-0.5`}>
      <Icon className="w-2.5 h-2.5" />
