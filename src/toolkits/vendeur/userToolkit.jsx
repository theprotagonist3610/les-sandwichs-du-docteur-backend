/*
vendeur/userToolkit.jsx
Gestion des accès Firestore et RTDB pour un utilisateur vendeur

Redirection par défaut : /ventes
*/
...
export async function loginUser(
  ...
  try {
    ...
    if (userData.role !== "vendeur") {
      await firebaseSignOut(auth);
      throw new Error(
        "Accès refusé : ce compte n'est pas un compte vendeur"
      );
    }
...
export async function logoutUser(navigate, redirectPath = "/login") {
  try {
    ...
    return {
      success: true,
      message: "Déconnexion vendeur réussie",
    };
  } catch (error) {
    throw error;
  }
